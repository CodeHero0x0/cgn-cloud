...
import com.cgn.framework.dbswitch.common.util.DbswitchStrUtils;
import com.cgn.framework.dbswitch.common.util.HivePrepareUtils;
import com.cgn.framework.dbswitch.common.util.StringUtil;
import com.cgn.framework.dbswitch.common.util.TypeConvertUtils;
import com.cgn.framework.dbswitch.core.model.*;
import com.cgn.framework.dbswitch.core.util.SqlEngineUtil;
...
public abstract class AbstractDatabase implements IDatabaseInterface {
    ...
    @Override
    public int queryTableDataCount(Connection connection, String tableName) {
        try {
    ...
    public boolean dropTable(Connection connection, String tableName) {
        try {
            Statement statement = connection.createStatement();
            String countSql = "DROP TABLE IF EXISTS " + tableName;
            return statement.execute(countSql);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
     ...
     * @throws SQLException
     */
    private void setIndex(List<ColumnDescription> columnDescriptions, ResultSet indexInfo) throws SQLException {
    ...
    public JdbcSelectResult queryDataBySql(Connection connection, String dbType, String sql, Integer openTrans, int rowCount) {
        ...
        try {
            ...
            //兼容doris
            if (ProductTypeEnum.DORIS.name().equalsIgnoreCase(dbType) && (sql.contains("create") || sql.contains("CREATE"))) {
                String[] statements = com.cgn.framework.dbswitch.core.util.SqlUtil.getStatements(sql, ";");
    ...
    public long countBySql(Connection connection, String countSql) {
        try {
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery(countSql);
            resultSet.next();
    ...
    public Integer executeBatchInsertSql(Connection connection, String sql) {
        try {
            connection.setAutoCommit(false);
            Statement statement = connection.createStatement();
